...
We can also incorporate model weights. This example uses random weights.

```{r train-models-weights}
train_test <- partition(cal_data, 0.7, 168, 5)

train_data <- train_test$train
train_data$random_weights <- runif(length(train_data$pm2_5_cf_ave))
...
train_models(formulas, train_data, train_data$random_weights)
```
